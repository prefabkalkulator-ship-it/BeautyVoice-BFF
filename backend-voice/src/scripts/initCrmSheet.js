...
const GoogleWorkspaceClient_1 = require("../services/GoogleWorkspaceClient");
const env_1 = require("../config/env");
async function initializeSheet() {
    console.log('Rozpoczynam inicjalizację arkusza CRM...');
    try {
        const isConnected = await GoogleWorkspaceClient_1.googleClient.testConnection();
        if (!isConnected) {
            console.error('Brak połączenia. Upewnij się, że plik credentials.json jest w katalogu backend-voice oraz że konto serwisowe ma dostęp (uprawnienie Edytor) do arkusza o ID:', env_1.config.googleSheetId);
            process.exit(1);
        }
...
initializeSheet();
//# sourceMappingURL=initCrmSheet.js.map