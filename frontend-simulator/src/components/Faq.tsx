        ...
        <div className="space-y-4 animate-in slide-in-from-right-4">
          ...
          <div className="glass-card p-3 sm:p-4 rounded-2xl mb-6 border border-surface-200/70 shadow-xs flex flex-col sm:flex-row gap-3 items-stretch sm:items-center justify-between">
            <div className="relative flex-1">
              ...
              <input 
                type="text"
                placeholder="Szukaj w pytaniach i odpowiedziach bazy wiedzy..."
                value={searchQuery}
                onChange={e => setSearchQuery(e.target.value)}
                className="w-full pl-10 pr-9 py-2.5 bg-surface-50 border border-surface-200 rounded-xl text-xs font-medium focus:ring-2 focus:ring-gold-500/20 focus:border-gold-500 transition"
              />
              {searchQuery && (
                <button 
                  type="button"
                  onClick={() => setSearchQuery('')}
                  className="absolute right-3 top-1/2 -translate-y-1/2 text-surface-400 hover:text-surface-600 p-1 rounded-full hover:bg-surface-200/50 cursor-pointer"
            ...
            <div className="flex items-center gap-2 self-start sm:self-center shrink-0">
              <button
                ...
                className={`px-3 py-2 rounded-xl text-xs font-semibold transition cursor-pointer ${
                  filterType === 'all' 
                    ...
                    : 'bg-surface-100 text-surface-600 hover:bg-surface-200/70'
                }`}
              >
              ...
              >
                ...
                <span>Poufne PIN ({faqs.filter(f => f.isConfidential).length})</span>
              </button>
            </div>
          </div>

          {filteredFaqs.length === 0 && faqs.length > 0 && (
            <div className="text-center py-10 glass-card rounded-2xl border border-dashed border-surface-300 p-6 mb-6">
              <Search className="w-8 h-8 text-surface-300 mx-auto mb-2" />
              <p className="text-sm font-semibold text-surface-700">Brak wyników wyszukiwania</p>
              <p className="text-xs text-surface-500 mt-1">
                {filterType === 'confidential' 
                  ? 'Brak wpisów oznaczonych jako poufne pasujących do podanego zapytania.' 
                  : `Nie znaleziono wpisów pasujących do frazy „${searchQuery}”.`}
              </p>
              <button 
                type="button"
                onClick={() => { setSearchQuery(''); setFilterType('all'); }} 
                className="mt-3 px-3 py-1.5 bg-surface-100 hover:bg-surface-200 text-surface-700 rounded-lg text-xs font-medium transition cursor-pointer"
              >
                Wyczyść filtry
              </button>
            </div>
